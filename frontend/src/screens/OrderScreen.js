...
const OrderScreen = () => {
  ...
  if (!loading) {
    ...
    tempOrder.itemsPrice = addDecimals(
      tempOrder.orderItems.reduce((acc, item) => acc + item.price * item.qty, 0)
    );
  }
  ...
  }, [dispatch, orderId, userInfo, successPay, order, navigate]);
